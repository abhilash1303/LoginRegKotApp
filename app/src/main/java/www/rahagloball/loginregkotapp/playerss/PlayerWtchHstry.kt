...
import www.rahagloball.loginregkotapp.models.watchistry.WtchHstryChanel;
import www.rahagloball.loginregkotapp.models.watchistry.WtchHstryComment;
import www.rahagloball.loginregkotapp.models.watchistry.WtchHstrySupprt;
import www.rahagloball.loginregkotapp.models.watchistry.WtchHstryVideo;
import www.rahagloball.loginregkotapp.models.watchistry.WtchHstryView;
import www.rahagloball.loginregkotapp.models.watchistry.WtchHstryVote;
...
class PlayerWtchHstry : AppCompatActivity(), SwipeInterface {
    ...
    var video: WtchHstryVideo? = null
    var channell: WtchHstryChanel? = null
    ...
    protected override fun onCreate(savedInstanceState: Bundle?) {
        ...
        contextt = this@PlayerWtchHstry
        manager = SessionManager()
        token = manager?.getPreferences(this@PlayerWtchHstry, Constants.USER_TOKEN_LRN)
        suprt_hit_str = manager?.getPreferences(this@PlayerWtchHstry, Constants.SUPPORT_HIT)
...
//        rl_vid_cntrlr = exoPlayerView?.findViewById<RelativeLayout>(R.id.rl_vid_cntrlr)
        ...
        saved_ivd_strrr = manager?.getPreferences(this@PlayerWtchHstry, Constants.VIDEO_SAVEEE)
        ...
        vid_speedddd?.setOnClickListener(View.OnClickListener { view: View? ->
            val builder = AlertDialog.Builder(this@PlayerWtchHstry)
...
//        Objects.requireNonNull(getSupportActionBar()).setDisplayHomeAsUpEnabled(true);
        ...
        try {
            ...
            if (data != null) {
                video = data.getSerializable("videoData") as WtchHstryVideo
            }
            ...
            //            Log.e("ch_imgimgimg", ch_imgimgimg);
//            https://nationlearns.com/images/channel/1665233851Chrysanthemum.jpg
            checImgSt = Configs.BASE_URL21 + "images/channel/" + channell?.image
            //            Log.e("checImgSt", checImgSt);
//            Glide.with(contextt).load(checImgSt).into(channel_pic_sub)
...
//                    .into(channel_pic_sub);
            val suptrs_cntt2: List<WtchHstrySupprt>? = channell?.supports
            ...
            ll_ch_viewww?.setOnClickListener(View.OnClickListener { view: View? ->
                val intent = Intent(this@PlayerWtchHstry, ChannelProfileActivity::class.java)
                intent.putExtra("ch_iddCuties", ch_idd_strtr)
                startActivity(intent)
            })
            val votesItems: List<WtchHstryVote>? = video?.votes
            if (votesItems != null) {
            ...
            val mySprtViewsList: List<WtchHstryView>? = video?.views
            val mySprtVsList = mySprtViewsList?.size
            views_vid?.text=("$mySprtVsList Views")
            val cmts_cnt_lsit: List<WtchHstryComment>? = video?.comments
            val cmts_cnt_str = cmts_cnt_lsit?.size
            comnts_cnt?.text=(cmts_cnt_str.toString())
            val lisescntstr1: List<WtchHstryVote>? = video?.votes
            lisescntstr = java.lang.String.valueOf(video!!.votes).toInt()
            ...
            comment_here?.setOnClickListener(View.OnClickListener { v: View? ->
                ...
                rv_commnetss =
                    bottomSheetLayout!!.findViewById<RecyclerView>(R.id.rv_commnetss)
...
//                rv_commnetss.setHasFixedSize(true);
                layoutManager = LinearLayoutManager(this@PlayerWtchHstry)
...
//                ll_super_spprt.visibility = (View.GONE);
                        ...
                        try {
                            ...
                            if (ss_stus != null) {
                                if (ss_stus.contains("1")) {
                                    ...
                                    mBottomSheetDialog = BottomSheetDialog(contextt as PlayerWtchHstry)
                                    mBottomSheetDialog!!.setContentView(bottomSheetLayout!!)
                                    mBottomSheetDialog!!.show()
                                } else if (ss_stus != null) {
                                    if (ss_stus.contains("0")) {
                                        rankDialog =
                                            Dialog(this@PlayerWtchHstry, R.style.FullHeightDialog)
            ...
            ll_report_vid?.setOnClickListener(View.OnClickListener { v: View? ->
                rankDialog = Dialog(this@PlayerWtchHstry, R.style.FullHeightDialog)
            ...
            ?.enqueue(object : GlobalCallback<String?>(channel_name11) {
                override fun onResponse(call: Call<String?>, response: Response<String?>) {
                    ...
                    manager?.setPreferences(
                        this@PlayerWtchHstry, Constants.VIDEO_SAVEEE,
                        saved_vid_res!!
                    )
                    if (saved_vid_res!!.contains("1")) {
                        // successful
                        Toast.makeText(this@PlayerWtchHstry, "Video is saved!!", Toast.LENGTH_SHORT)
                            .show()
                        ll_savedd_vid?.visibility = (View.VISIBLE)
                        ll_save_vid?.visibility = (View.GONE)
...
//                                    cfPaymentService.doPayment(PlayerWtchHstry.this, params, token1, "PROD", "#174778", "#FFFFFF", false);
////                                    Intent intentpay = new Intent(PlayerWtchHstry.this, PlayerWtchHstry.class);
////                                    startActivity(intentpay);
//
//                                } else {
//                                    Toast.makeText(PlayerWtchHstry.this, "msg2" + response.body().getMessage(), Toast.LENGTH_SHORT).show();
//                                }
//                            } else {
//                                Toast.makeText(PlayerWtchHstry.this, "msg3" + response.body().getMessage(), Toast.LENGTH_SHORT).show();
//                            }
//
//
//                        } catch (Exception e) {
...
//                });
    ...
    private fun getLEarnVids(subcatIdStr: String) {
        ...
        RetrofitClient.getClient().rellvidList(
            ...
            ?.enqueue(object : GlobalCallback<RelatedVidPojo?>(PlayerWtchHstry.rv_all_videos) {
                @SuppressLint("SetTextI18n")
                override fun onResponse(
                    ...
                    try {
                        ...
                        if (catggryList != null) {
                            if (catggryList.isEmpty()) {
                                PlayerWtchHstry.rv_all_videos!!.visibility = View.GONE
                            } else {
                                PlayerWtchHstry.rv_all_videos!!.visibility = View.VISIBLE
                                PlayerWtchHstry.adapter_vids_list =
                                    RelVidListAdapter(catggryList, this@PlayerWtchHstry)
                                PlayerWtchHstry.rv_all_videos!!.adapter =
                                    PlayerWtchHstry.adapter_vids_list
                            }
                        }
                    } catch (e: Exception) {
...
//        blur_reg1.visibility = (View.VISIBLE);
            ...
            RetrofitClient.getClient().allvidList(
                ...
                ?.enqueue(object : GlobalCallback<OneVidListPojo?>(PlayerWtchHstry.rv_commnetss) {
                    @SuppressLint("SetTextI18n")
                    override fun onResponse(
                        ...
                        try {
                            ...
                            if (catggryList != null) {
                                if (catggryList.isEmpty()) {
                                    ...
                                    PlayerWtchHstry.rv_commnetss!!.visibility = View.VISIBLE
                                    PlayerWtchHstry.adapter = catggryList.let {
                                        YtCommentsAdapter(
                                            it,
                                            this@PlayerWtchHstry
                                        )
                                    }
                                    PlayerWtchHstry.rv_commnetss!!.adapter = PlayerWtchHstry.adapter
                                }
                            }
                        } catch (e: Exception) {
                            e.printStackTrace()
        ...
        RetrofitClient.getClient().hitsuppportt(chh_iddstr, "application/json", "Bearer $token")
            ?.enqueue(object : GlobalCallback<SupporrtPojo?>(title) {
                ...
                override fun onResponse(
                    ...
                    if (res != null) {
                        manager?.setPreferences(this@PlayerWtchHstry, Constants.SUPPORT_HIT, res)
                    }
                    if (res != null) {
                        if (res.contains("2")) {
