...
import www.rahagloball.loginregkotapp.models.singlecutiesch.SinCtsChCmnt
import www.rahagloball.loginregkotapp.models.singlecutiesch.SingleCutiesCh

class YtCommentsAdapterCutCh(data: List<SingleCutiesCh>, mContext: Context) :
    RecyclerView.Adapter<YtCommentsAdapterCutCh.MyViewHolder>() {
    private val dataSet: List<SingleCutiesCh>
...
//        String commnnts_body_str = dataSet.getBody();
            val vidlist: SingleCutiesCh = dataSet[listPosition]

//            CutChannel channell = vidlist.getChannel();
//
//            String checImgSt = "https://bucket-nationlearns.s3.ap-south-1.amazonaws.com/images/pool/video/" + channell.getImage();
//            String checNmae = channell.getName();
//            user_cmnt_name.setText(checNmae);


//            Picasso.get().load(checImgSt).into(user_cmnt_pic);
            val commentsItemList: List<SinCtsChCmnt>? = vidlist.comments
            if (commentsItemList != null) {
                for (i in commentsItemList.indices) {
                    val commentsItem: SinCtsChCmnt = commentsItemList[i]
