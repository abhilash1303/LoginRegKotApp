...
class SupportersList constructor() : AppCompatActivity() {
    ...
    protected override fun onCreate(savedInstanceState: Bundle?) {
        ...
        token = manager?.getPreferences(this@SupportersList, Constants.USER_TOKEN_LRN)
        ...
        RetrofitClient.getClient().getSuprtrsLsit(url, "application/json", "Bearer " + token)
            ?.enqueue(object : GlobalCallback<SupportersListPojo?>(rv_supporters) {
             ...
             override   fun onResponse(
                    ...
                    try {
                        val subscriptionList: List<Supporter> ? = response.body()?.supporterList
                        if (subscriptionList != null) {
                            if (subscriptionList.isEmpty()) {
                                rv_supporters!!.visibility = View.GONE
                                nodata?.visibility = View.VISIBLE
                            } else {
                                adapter = SupportersListAdapter(subscriptionList, this@SupportersList)
